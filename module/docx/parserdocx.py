...
filename = 'docForparse\Требования к образованию Здравоохранение.docx'
...
def main():
	for table in tables:
		table.columns[0].cells[0].text = 'title_spec'
		table.columns[1].cells[0].text = 'old_treb'
		table.columns[2].cells[0].text = 'new_treb'
		# table.columns[3].cells[0].text = 'price'
		# table.columns[4].cells[0].text = 'date'
		# table.columns[5].cells[0].text = 'linkNmo'
		...
		print(f'items {items}')
	try:
		with open(f'{os.getcwd()}\\data\\json\\docx_.json', 'w', encoding='utf-8') as file:
